      ...
      for (t3=max(ceild(32*t2-_PB_N-28,32),2*t1);t3<=min(min(floord(2*_PB_TSTEPS+_PB_N-3,32),floord(64*t1+_PB_N+61,32)),floord(32*t2+_PB_N+28,32));t3++) {
        if ((t1 <= floord(32*t3-_PB_N+1,64)) && (t2 <= t3-1)) {
          if ((_PB_N+1)%2 == 0) {
            for (t5=max(32*t2,32*t3-_PB_N+3);t5<=32*t2+31;t5++) {
              A[(-32*t3+t5+_PB_N-2)][(_PB_N-2)] = val * (B[(-32*t3+t5+_PB_N-2)][(_PB_N-2)] + B[(-32*t3+t5+_PB_N-2)][(_PB_N-2) - 1] + B[(-32*t3+t5+_PB_N-2)][1 + (_PB_N-2)] + B[1 + (-32*t3+t5+_PB_N-2)][(_PB_N-2)] + B[(-32*t3+t5+_PB_N-2) - 1][(_PB_N-2)]);;
            }
          }
        }
        if ((t1 <= floord(32*t2-_PB_N+1,64)) && (t2 >= t3)) {
          if ((_PB_N+1)%2 == 0) {
            for (t6=max(32*t3,32*t2-_PB_N+3);t6<=min(32*t2,32*t3+31);t6++) {
              A[(_PB_N-2)][(-32*t2+t6+_PB_N-2)] = val * (B[(_PB_N-2)][(-32*t2+t6+_PB_N-2)] + B[(_PB_N-2)][(-32*t2+t6+_PB_N-2) - 1] + B[(_PB_N-2)][1 + (-32*t2+t6+_PB_N-2)] + B[1 + (_PB_N-2)][(-32*t2+t6+_PB_N-2)] + B[(_PB_N-2) - 1][(-32*t2+t6+_PB_N-2)]);;
            }
          }
        }
        if ((_PB_N == 3) && (t2 == t3)) {
          for (t4=16*t2;t4<=min(min(_PB_TSTEPS-1,32*t1+31),16*t2+14);t4++) {
            B[1][1] = val * (A[1][1] + A[1][1 - 1] + A[1][1 + 1] + A[1 + 1][1] + A[1 - 1][1]);;
            A[1][1] = val * (B[1][1] + B[1][1 - 1] + B[1][1 + 1] + B[1 + 1][1] + B[1 - 1][1]);;
          }
        }
        if (t2 == t3) {
          for (t4=max(ceild(32*t2-_PB_N+2,2),32*t1);t4<=min(min(min(floord(32*t2-_PB_N+32,2),_PB_TSTEPS-1),32*t1+31),16*t2-1);t4++) {
            for (t5=32*t2;t5<=2*t4+_PB_N-2;t5++) {
              for (t6=32*t2;t6<=2*t4+_PB_N-2;t6++) {
                B[(-2*t4+t5)][(-2*t4+t6)] = val * (A[(-2*t4+t5)][(-2*t4+t6)] + A[(-2*t4+t5)][(-2*t4+t6) - 1] + A[(-2*t4+t5)][1 + (-2*t4+t6)] + A[1 + (-2*t4+t5)][(-2*t4+t6)] + A[(-2*t4+t5) - 1][(-2*t4+t6)]);;
                A[(-2*t4+t5-1)][(-2*t4+t6-1)] = val * (B[(-2*t4+t5-1)][(-2*t4+t6-1)] + B[(-2*t4+t5-1)][(-2*t4+t6-1) - 1] + B[(-2*t4+t5-1)][1 + (-2*t4+t6-1)] + B[1 + (-2*t4+t5-1)][(-2*t4+t6-1)] + B[(-2*t4+t5-1) - 1][(-2*t4+t6-1)]);;
              }
              A[(-2*t4+t5-1)][(_PB_N-2)] = val * (B[(-2*t4+t5-1)][(_PB_N-2)] + B[(-2*t4+t5-1)][(_PB_N-2) - 1] + B[(-2*t4+t5-1)][1 + (_PB_N-2)] + B[1 + (-2*t4+t5-1)][(_PB_N-2)] + B[(-2*t4+t5-1) - 1][(_PB_N-2)]);;
            }
            for (t6=32*t2;t6<=2*t4+_PB_N-1;t6++) {
              A[(_PB_N-2)][(-2*t4+t6-1)] = val * (B[(_PB_N-2)][(-2*t4+t6-1)] + B[(_PB_N-2)][(-2*t4+t6-1) - 1] + B[(_PB_N-2)][1 + (-2*t4+t6-1)] + B[1 + (_PB_N-2)][(-2*t4+t6-1)] + B[(_PB_N-2) - 1][(-2*t4+t6-1)]);;
            }
          }
        }
        ...
        if ((_PB_N >= 4) && (t2 == t3)) {
          for (t4=16*t2;t4<=min(min(floord(32*t2-_PB_N+32,2),_PB_TSTEPS-1),32*t1+31);t4++) {
            for (t6=2*t4+1;t6<=2*t4+_PB_N-2;t6++) {
              B[1][(-2*t4+t6)] = val * (A[1][(-2*t4+t6)] + A[1][(-2*t4+t6) - 1] + A[1][1 + (-2*t4+t6)] + A[1 + 1][(-2*t4+t6)] + A[1 - 1][(-2*t4+t6)]);;
            }
            for (t5=2*t4+2;t5<=2*t4+_PB_N-2;t5++) {
              B[(-2*t4+t5)][1] = val * (A[(-2*t4+t5)][1] + A[(-2*t4+t5)][1 - 1] + A[(-2*t4+t5)][1 + 1] + A[1 + (-2*t4+t5)][1] + A[(-2*t4+t5) - 1][1]);;
        ...
        if ((t1 >= ceild(t2-1,2)) && (t2 <= min(floord(_PB_TSTEPS-16,16),t3-1))) {
          for (t6=32*t3;t6<=min(32*t3+31,32*t2+_PB_N+28);t6++) {
            B[1][(-32*t2+t6-30)] = val * (A[1][(-32*t2+t6-30)] + A[1][(-32*t2+t6-30) - 1] + A[1][1 + (-32*t2+t6-30)] + A[1 + 1][(-32*t2+t6-30)] + A[1 - 1][(-32*t2+t6-30)]);;
          }
        }
        if ((t1 >= ceild(t3-1,2)) && (t2 >= t3) && (t3 <= floord(_PB_TSTEPS-16,16))) {
          for (t5=max(32*t2,32*t3+31);t5<=min(32*t2+31,32*t3+_PB_N+28);t5++) {
            B[(-32*t3+t5-30)][1] = val * (A[(-32*t3+t5-30)][1] + A[(-32*t3+t5-30)][1 - 1] + A[(-32*t3+t5-30)][1 + 1] + A[1 + (-32*t3+t5-30)][1] + A[(-32*t3+t5-30) - 1][1]);;
          }
        }
      }
    }
