...
compile_one()
{
  ...
  $TIMEOUT ./taffo.sh \
    ...
    -float-output build/"$benchname".orgion.out \
    -temp-dir build \
    "$benchpath" \
...
compile_pluto()
{
  ...
  $TIMEOUT ./taffo.sh \
    -o build/"$benchname".out \
    -float-output build/"$benchname".pluto.out \
    -temp-dir build \
    "$benchpath" \
    ./utilities/polybench.c \
    -I"$benchdir" \
    ...
    -debug-taffo \
    -lm \
    2> build/${benchname}_pluto.log || return $?
}
...
for bench in $all_benchs; do
  ...
  if [[ "$bench" =~ $ONLY ]]; then
    ...
    opts=$(read_opts ${bench})
    pluto=${bench%%.c*}
    printf '[....] %s' "${pluto}.pluto.c"
    compile_pluto "${pluto}.pluto.c" \
      "-O3 \
      -DPOLYBENCH_TIME -DPOLYBENCH_DUMP_ARRAYS -DPOLYBENCH_STACK_ARRAYS \
      -D$D_CONF -D$D_LARGE_DATASET $MIXIMODE \
      -Xdta -totalbits -Xdta $TOT \
      $ERRORPROP $opts"
    bpid_fc=$?
    if [[ $bpid_fc == 0 ]]; then
