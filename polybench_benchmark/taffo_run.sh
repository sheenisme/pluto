...
run_one()
{
  ...
  fix_out=build/"$benchname".pluto.out
  ...
  sed -i "/==BEGIN DUMP_ARRAYS==/d"  $datadir/$benchname.pluto.csv 
  sed -i "/==END   DUMP_ARRAYS==/d"  $datadir/$benchname.pluto.csv
  sed -i "/begin dump/d"  $datadir/$benchname.pluto.csv 
  sed -i "/end   dump/d"  $datadir/$benchname.pluto.csv
}
