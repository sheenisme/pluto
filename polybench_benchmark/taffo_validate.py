...
def ComputeSpeedups(float_times, fixp_times):
  ...
  return {'plc_t': fixp_avg, 'org_t': float_avg, 'speedup': speedup}
...
if __name__ == "__main__":
  ...
  for bench in BenchmarkList():
    ...
    float_dataf = PolybenchRootDir() / 'results-out' / (name+'.origion.csv')
    float_data = ReadValues(str(float_dataf))
    float_timesf = PolybenchRootDir() / 'results-out' / (name+'.origion.time.txt')
    float_times = ReadValues(str(float_timesf))
    fixp_dataf = PolybenchRootDir() / 'results-out' / (name+'.pluto.csv')
    fixp_data = ReadValues(str(fixp_dataf))
    fixp_timesf = PolybenchRootDir() / 'results-out' / (name+'.pluto.time.txt')
